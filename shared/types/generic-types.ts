...
// Re-export generic types from llm-service for convenience
export type {
  // Core interfaces
  LLMProvider,
  LLMProviderFactory,
...
// Re-export registry types (MVP - simplified)
export type {
...
export {
  ...
  shutdownGlobalProviderRegistry,
} from '../services/provider-registry.js';

// Import existing types for mapping
import {
  Assistant,
 ...
 */
export function mapOpenAIToGenericAssistant(openaiAssistant: Assistant): GenericAssistant {
  return {
 ...
 */
export function mapGenericToOpenAIAssistant(genericAssistant: GenericAssistant): Assistant {
 ...
 */
export function mapOpenAIToGenericTool(openaiTool: Assistant['tools'][0]): GenericTool {
  return {
 ...
 */
export function mapGenericToOpenAITool(genericTool: GenericTool): Assistant['tools'][0] {
  if (genericTool.providerData) {
    return genericTool.providerData;
  }

  return {
    type: genericTool.type as 'code_interpreter' | 'file_search' | 'function',
 ...
 */
export function mapOpenAIToGenericThread(openaiThread: Thread): GenericThread {
  return {
    id: openaiThread.id,
 ...
 */
export function mapGenericToOpenAIThread(genericThread: GenericThread): Thread {
 ...
 */
export function mapOpenAIToGenericMessage(openaiMessage: Message): GenericMessage {
  return {
    id: openaiMessage.id,
 ...
 */
export function mapGenericToOpenAIMessage(genericMessage: GenericMessage): Message {
 ...
 */
export function mapOpenAIToGenericMessageContent(openaiContent: Message['content'][0]): GenericMessageContent {
  const genericContent: GenericMessageContent = {
 ...
 */
export function mapGenericToOpenAIMessageContent(genericContent: GenericMessageContent): Message['content'][0] {
 ...
 */
export function mapOpenAIToGenericRun(openaiRun: Run): GenericRun {
  return {
    id: openaiRun.id,
 ...
 */
export function mapGenericToOpenAIRun(genericRun: GenericRun): Run {
  ...
  return {
    ...
    last_error: genericRun.lastError ? {
      code: genericRun.lastError.code as 'server_error' | 'rate_limit_exceeded' | 'invalid_prompt',
 ...
 */
export function mapOpenAIToGenericRunStep(openaiRunStep: RunStep): GenericRunStep {
  return {
    id: openaiRunStep.id,
 ...
 */
export function mapGenericToOpenAIRunStep(genericRunStep: GenericRunStep): RunStep {
  ...
  return {
    ...
    type: genericRunStep.type as 'message_creation' | 'tool_calls',
    ...
    last_error: genericRunStep.lastError ? {
      code: genericRunStep.lastError.code as 'server_error' | 'rate_limit_exceeded',
 ...
 */
export function mapOpenAIToGenericCreateAssistantRequest(
  openaiRequest: CreateAssistantRequest
): GenericCreateAssistantRequest {
  return {
 ...
 */
export function mapGenericToOpenAICreateAssistantRequest(
  genericRequest: GenericCreateAssistantRequest
): CreateAssistantRequest {
 ...
 */
export function mapOpenAIToGenericListResponse<T, U>(
  openaiResponse: { object: 'list'; data: T[]; has_more: boolean; first_id?: string; last_id?: string },
  mapper: (item: T) => U
): GenericListResponse<U> {
  return {
 ...
 */
export function mapGenericToOpenAIListResponse<T, U>(
  genericResponse: GenericListResponse<T>,
  mapper: (item: T) => U
): { object: 'list'; data: U[]; has_more: boolean; first_id?: string; last_id?: string } {
  return {
    object: 'list',
...
/**
 * Utility type for provider-specific data preservation
 */
export interface ProviderDataContainer<T = any> {
  providerData?: T;
}
 ...
 */
export function preserveProviderData<T extends ProviderDataContainer, U extends ProviderDataContainer>(
  source: T,
  target: U
): U {
