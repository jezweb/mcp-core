...
export interface ProviderRegistryConfig {
  ...
  // /** Maximum number of retry attempts for failed providers */
...
export class ProviderRegistry {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new MCPError(
        ...
        `Failed to initialize provider registry: ${error instanceof Error ? error.message : 'Unknown error'}`,
        { originalError: error }
      );
    }
  }
   ...
   */
  async registerProvider(provider: LLMProvider, config?: LLMProviderConfig): Promise<void> {
  ...
  private async initializeProvider(config: LLMProviderConfig): Promise<void> {
    ...
    if (!factory) {
      throw new MCPError(
        ...
        `Provider factory not found: ${config.provider}`
      );
    }
  ...
  // MVP: Advanced features - implement later
  // private selectByPriority(providers: LLMProvider[]): LLMProvider | undefined {
  //   let highestPriority = -1;
  //   let selectedProvider: LLMProvider | undefined;

  //   for (const provider of providers) {
  //     const config = this.configs.get(provider.metadata.name);
  ...
  // private selectRoundRobin(providers: LLMProvider[]): LLMProvider | undefined {
  ...
  // private selectByCapabilities(providers: LLMProvider[], requiredCapabilities: string[]): LLMProvider | undefined {
  //   for (const provider of providers) {
  //     const capabilities = provider.metadata.capabilities;
  //     const hasAllCapabilities = requiredCapabilities.every(cap => {
  //       return (capabilities as any)[cap] === true;
  //     });
  ...
  // private selectByHealth(providers: LLMProvider[]): LLMProvider | undefined {
  //   let healthiestProvider: LLMProvider | undefined;
  //   let latestHealthCheck = new Date(0);

  //   for (const provider of providers) {
  //     const status = this.status.get(provider.metadata.name);
  //     if (status?.healthy && status.lastHealthCheck && status.lastHealthCheck > latestHealthCheck) {
  //       latestHealthCheck = status.lastHealthCheck;
  ...
  //       await this.performHealthCheck(providerName, provider);
  ...
  // private async performHealthCheck(providerName: string, provider: LLMProvider): Promise<void> {
  //   try {
  //     const isHealthy = await provider.validateConnection();
  //     const currentStatus = this.status.get(providerName);
  ...
  //     if (!currentStatus || currentStatus.healthy !== isHealthy) {
  //       this.status.set(providerName, newStatus);
  //       this.emit('provider:status-changed', providerName, newStatus);
  //     }
  //   } catch (error) {
  //     const newStatus: LLMProviderStatus = {
  //       name: providerName,
  ...
  // private emit<K extends keyof ProviderRegistryEvents>(event: K, ...args: Parameters<ProviderRegistryEvents[K]>): void {
  //   const listeners = this.eventListeners.get(event);
  //   if (listeners) {
...
export function getGlobalProviderRegistry(): ProviderRegistry {
  if (!globalRegistry) {
    throw new MCPError(
      ErrorCodes.INTERNAL_ERROR,
      'Global provider registry not initialized. Call initializeGlobalProviderRegistry() first.'
    );
  }
 ...
 */
export function initializeGlobalProviderRegistry(config: ProviderRegistryConfig): ProviderRegistry {
  if (globalRegistry) {
    throw new MCPError(
      ErrorCodes.INTERNAL_ERROR,
      'Global provider registry already initialized'
    );
  }
 ...
 */
export async function shutdownGlobalProviderRegistry(): Promise<void> {
  if (globalRegistry) {
