...
import { EnvironmentManager, EnvironmentDetectionResult } from './environment-manager.js';
...
export interface HealthCheck {
  name: string;
  category: 'configuration' | 'environment' | 'deployment' | 'security' | 'performance';
...
export class DeploymentHealthChecker {
   ...
   */
  private async checkEnvironmentDetection(detection: EnvironmentDetectionResult): Promise<HealthCheck[]> {
    const checks: HealthCheck[] = [];
   ...
   */
  private async checkConfiguration(config: MCPServerConfig, environment: string): Promise<HealthCheck[]> {
    const checks: HealthCheck[] = [];
   ...
   */
  private async checkSecurity(config: MCPServerConfig, environment: string): Promise<HealthCheck[]> {
    const checks: HealthCheck[] = [];
   ...
   */
  private async checkPerformance(config: MCPServerConfig, environment: string): Promise<HealthCheck[]> {
    const checks: HealthCheck[] = [];
   ...
   */
  private async checkApiConnectivity(config: MCPServerConfig): Promise<HealthCheck[]> {
    const checks: HealthCheck[] = [];
   ...
   */
  private checkCloudflareWorkerLimits(config: MCPServerConfig): HealthCheck[] {
    const checks: HealthCheck[] = [];
  ...
  private generateRecommendations(
    ...
    if (environment === 'production') {
      recommendations.push(
        'Ensure all security configurations are properly set for production',
        'Monitor performance metrics and adjust limits as needed',
        'Set up proper logging and monitoring for production deployment'
      );
    }

    if (environment === 'development') {
      recommendations.push(
        'Enable debug logging and hot reload for better development experience',
        'Consider using lower resource limits to match production constraints'
      );
    }
   ...
   * Helper methods
   */
  private getExpectedDeploymentType(environment: string): string {
    switch (environment) {
      case 'production':
        return 'cloudflare';
      case 'staging':
  ...
  private getExpectedTransport(deployment: string): string {
    switch (deployment) {
      case 'cloudflare':
        return 'http';
      case 'npm':
  ...
  private getRequiredFields(environment: string): string[] {
    const baseFields = [
      'server.name',
      'server.version',
      'server.environment',
  ...
  private hasConfigValue(config: any, path: string): boolean {
  ...
  private getRecommendedConcurrencyLimits(environment: string): { min: number; max: number } {
    switch (environment) {
      case 'production':
 ...
 */
let globalHealthChecker: DeploymentHealthChecker | null = null;

/**
 * Get or create the global health checker
 */
export function getGlobalHealthChecker(): DeploymentHealthChecker {
