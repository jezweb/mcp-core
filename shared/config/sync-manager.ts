...
export interface ConfigurationConflict {
  ...
  localValue: any;
  remoteValue: any;
  localVersion: ConfigurationVersion;
  remoteVersion: ConfigurationVersion;
...
export interface PubSubMessage {
  type: 'config-update' | 'version-update' | 'peer-discovery' | 'heartbeat';
  instanceId: string;
  timestamp: string;
  payload: any;
  version?: ConfigurationVersion;
...
class ConfigurationVersionManager {
  private currentVersion: ConfigurationVersion;
  private versionHistory: ConfigurationVersion[] = [];
  ...
  getCurrentVersion(): ConfigurationVersion {
    return { ...this.currentVersion };
  }

  incrementVersion(config: MCPServerConfig, metadata?: Record<string, any>): ConfigurationVersion {
  ...
  compareVersions(v1: ConfigurationVersion, v2: ConfigurationVersion): number {
  ...
  isNewer(v1: ConfigurationVersion, v2: ConfigurationVersion): boolean {
 ...
 */
export class ConfigurationSyncManager {
  private instanceId: string;
  ...
  private syncCallbacks: Set<(event: SyncEvent) => void> = new Set();
   ...
   * Add sync peer
   */
  addPeer(peer: Omit<SyncPeer, 'lastSeen' | 'status'>): void {
   ...
   */
  async pushConfiguration(config?: MCPServerConfig): Promise<void> {
  ...
  async resolveConflict(
    ...
    const resolvedConflict: ConfigurationConflict = {
      ...conflict,
   ...
   */
  offSync(callback: (event: SyncEvent) => void): void {
   ...
   */
  private async handleLocalConfigurationChange(event: ConfigurationChangeEvent): Promise<void> {
   ...
   */
  private async handleRemoteConfigurationUpdate(message: PubSubMessage): Promise<void> {
    try {
      const remoteConfig = message.payload as MCPServerConfig;
      ...
      if (conflicts.length > 0) {
        ...
        for (const conflict of conflicts) {
          ...
          if (resolvedValue !== conflict) {
            ...
            await this.runtimeManager.updateConfigurationPath(
              conflict.path,
              resolvedValue,
   ...
   */
  private async handleVersionUpdate(message: PubSubMessage): Promise<void> {
   ...
   */
  private async handlePeerDiscovery(message: PubSubMessage): Promise<void> {
   ...
   * Update peer status
   */
  private updatePeerStatus(peerId: string, status: SyncPeer['status']): void {
 ...
 */
let globalSyncManager: ConfigurationSyncManager | null = null;
 ...
 */
export function setGlobalSyncManager(manager: ConfigurationSyncManager): void {
