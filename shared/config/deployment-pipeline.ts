...
import { DeploymentHealthChecker, HealthCheckResult } from './deployment-health.js';
...
export interface DeploymentStage {
  name: string;
  environment: string;
  deploymentType: 'cloudflare' | 'npm' | 'local';
  preDeployment: DeploymentStep[];
...
export interface DeploymentStep {
  name: string;
  type: 'command' | 'validation' | 'health-check' | 'config-update' | 'notification';
  command?: string;
  timeout?: number;
  retries?: number;
...
export interface HealthCheckConfig {
  name: string;
  type: 'http' | 'config' | 'deployment' | 'custom';
  endpoint?: string;
  timeout?: number;
  retries?: number;
...
export interface RollbackCondition {
  type: 'health-check-failure' | 'error-rate' | 'timeout' | 'manual';
  threshold?: number;
  duration?: number;
}
...
export interface NotificationEvent {
  type: 'deployment-start' | 'deployment-success' | 'deployment-failure' | 'rollback';
  channels: string[];
  template?: string;
}
...
export class DeploymentPipelineManager {
  ...
  async executeStage(
    ...
    await this.sendNotification(
      success ? 'deployment-success' : 'deployment-failure',
      result
    );
   ...
   */
  private async executeCommand(step: DeploymentStep, stage: DeploymentStage): Promise<string> {
    if (!step.command) {
   ...
   */
  private async executeStepValidation(step: DeploymentStep, stage: DeploymentStage): Promise<string> {
   ...
   */
  private async executeStepHealthCheck(step: DeploymentStep, stage: DeploymentStage): Promise<string> {
    const config = await this.environmentManager.loadEnvironmentConfig(stage.environment);
   ...
   */
  private async executeConfigUpdate(step: DeploymentStep, stage: DeploymentStage): Promise<string> {
   ...
   */
  private async executeNotificationStep(step: DeploymentStep, stage: DeploymentStage): Promise<string> {
   ...
   * Execute validation
   */
  private async executeValidation(stage: DeploymentStage, phase: string): Promise<StepResult> {
    const startTime = new Date();
    let success = true;
    let error: Error | undefined;
   ...
   */
  private async executeHealthChecks(stage: DeploymentStage): Promise<HealthCheckResult> {
    const config = await this.environmentManager.loadEnvironmentConfig(stage.environment);
   ...
   * Execute rollback
   */
  private async executeRollback(stage: DeploymentStage, deploymentError?: Error): Promise<RollbackResult> {
    const steps: StepResult[] = [];
    let success = true;
    let error: Error | undefined;
   ...
   */
  private async handleDeploymentFailure(result: DeploymentResult, stage: DeploymentStage): Promise<void> {
    // Log failure details
 ...
 */
export const defaultPipelineConfig: DeploymentPipelineConfig = {
 ...
 */
export function createDeploymentPipeline(config?: Partial<DeploymentPipelineConfig>): DeploymentPipelineManager {
  const mergedConfig = {
