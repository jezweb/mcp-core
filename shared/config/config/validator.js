...
class ConfigurationValidator {
    constructor() {
        this.schema = schema_js_1.mcpServerConfigSchema;
    }
    /**
    ...
    validateServer(server, errors, warnings) {
        ...
        else if (!(0, config_types_js_1.isValidEnvironment)(server.environment)) {
            errors.push({
                path: 'server.environment',
     ...
     */
    validateAPI(api, errors, warnings) {
        if (!api?.openai) {
            errors.push({
                path: 'api.openai',
     ...
     */
    validatePerformance(performance, errors, warnings) {
        if (!performance)
            return;
     ...
     */
    validateSecurity(security, errors, warnings) {
        if (!security)
            return;
        // CORS validation
     ...
     */
    validateDeployment(deployment, errors, warnings) {
        if (!deployment)
            return;
        if (!(0, config_types_js_1.isValidDeploymentType)(deployment.type)) {
            errors.push({
        ...
        if (!(0, config_types_js_1.isValidTransportType)(deployment.transport)) {
            errors.push({
        ...
        if (!(0, config_types_js_1.isValidLogLevel)(deployment.logLevel)) {
            errors.push({
     ...
     */
    async validateEnvironment(config, environment) {
