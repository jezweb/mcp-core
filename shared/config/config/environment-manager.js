...
exports.detectCurrentEnvironment = detectCurrentEnvironment;
exports.loadEnvironmentConfiguration = loadEnvironmentConfiguration;
...
class EnvironmentManager {
    ...
    detectEnvironment() {
        ...
        const result = detectionResults.reduce((best, current) => current.confidence > best.confidence ? current : best);
    ...
    async validateEnvironmentConfig(environment, config) {
        ...
        const environmentValidation = this.validateEnvironmentSpecificRules(environment, configToValidate, detection);
        return {
            ...baseValidation,
            environment,
            deployment: detection.deployment,
            configurationPath: `environments/${environment}.json`,
            recommendations: this.generateEnvironmentRecommendations(environment, configToValidate, detection),
        };
    }
    /**
     * Validate environment-specific rules
     */
    validateEnvironmentSpecificRules(environment, config, detection) {
        const errors = [];
        const warnings = [];
     ...
     */
    generateEnvironmentRecommendations(environment, config, detection) {
        const recommendations = [];
        // Environment mismatch recommendations
        if (detection.confidence < 0.8) {
            recommendations.push(`Environment detection confidence is low (${Math.round(detection.confidence * 100)}%). ` +
     ...
     */
    mergeWithOverrides(baseConfig, overrides) {
        const mergedConfig = (0, defaults_js_1.deepMerge)(baseConfig, overrides);
        // Update runtime manager if available
        if (this.runtimeManager) {
     ...
     */
    async applyEnvironmentConfigAtRuntime(environment) {
        if (!this.runtimeManager) {
     ...
     */
    async switchEnvironmentAtRuntime(newEnvironment) {
 ...
 */
let globalEnvironmentManager = null;
/**
 ...
 */
async function validateEnvironmentConfiguration(environment, config) {
