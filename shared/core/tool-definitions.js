 ...
 */
export function generateToolDefinitions(registry) {
    const tools = [];
    ...
    for (const toolName of registeredTools) {
        ...
        if (definition) {
            ...
            tools.push({
                name: toolName,
                title: toolName.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '),
 ...
 */
export function validateToolDefinitions(registry) {
    const registeredTools = registry.getRegisteredTools();
