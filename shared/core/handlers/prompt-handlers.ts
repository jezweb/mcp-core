...
export class PromptsListHandler extends BasePromptHandler {
  ...
  async execute(params: any): Promise<{ prompts: Prompt[]; nextCursor?: string }> {
    try {
...
export class PromptsGetHandler extends BasePromptHandler {
        ...
        if (typeof value !== 'string') {
          return {
            ...
            error: this.createValidationError(
              `Argument '${key}' must be a string, got ${typeof value}`,
              'arguments'
            )
          };
        }
  ...
  async execute(params: any): Promise<{ description?: string; messages: PromptMessage[] }> {
 ...
 */
export function createPromptHandlers(context: PromptHandlerContext) {
  return {
 ...
 */
export function getPromptHandlerMethods(): string[] {
 ...
 */
export function isPromptHandlerMethod(method: string): boolean {
