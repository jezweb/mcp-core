...
import { validateOpenAIId, validateModel, validateMetadata, validatePaginationParams, validateArray } from '../../validation/index.js';
/**
...
export class RunCreateHandler extends BaseToolHandler {
    ...
    async execute(args) {
        try {
            ...
            throw this.createExecutionError(`Failed to create run: ${error instanceof Error ? error.message : 'Unknown error'}`, error);
        }
    }
}
/**
...
export class RunListHandler extends BaseToolHandler {
    ...
    async execute(args) {
        try {
            ...
            throw this.createExecutionError(`Failed to list runs: ${error instanceof Error ? error.message : 'Unknown error'}`, error);
        }
    }
}
/**
...
export class RunGetHandler extends BaseToolHandler {
    ...
    async execute(args) {
        try {
            ...
            throw this.createExecutionError(`Failed to get run: ${error instanceof Error ? error.message : 'Unknown error'}`, error);
        }
    }
}
/**
...
export class RunUpdateHandler extends BaseToolHandler {
    ...
    async execute(args) {
        try {
            ...
            throw this.createExecutionError(`Failed to update run: ${error instanceof Error ? error.message : 'Unknown error'}`, error);
        }
    }
}
/**
...
export class RunCancelHandler extends BaseToolHandler {
    ...
    async execute(args) {
        try {
            ...
            throw this.createExecutionError(`Failed to cancel run: ${error instanceof Error ? error.message : 'Unknown error'}`, error);
        }
    }
}
/**
...
export class RunSubmitToolOutputsHandler extends BaseToolHandler {
        ...
        if (args?.tool_outputs && Array.isArray(args.tool_outputs)) {
            for (let i = 0; i < args.tool_outputs.length; i++) {
                ...
                if (!output.tool_call_id) {
                    return {
                        isValid: false,
                        error: new MCPError(ErrorCodes.INVALID_PARAMS, `Tool output at index ${i} is missing 'tool_call_id'. Each tool output must include the tool_call_id from the run's required_action. Example: {"tool_call_id": "call_abc123def456ghi789jkl012", "output": "result"}.`)
                    };
                }
                ...
                if (!output.output || typeof output.output !== 'string') {
                    return {
                        isValid: false,
                        error: new MCPError(ErrorCodes.INVALID_PARAMS, `Tool output at index ${i} is missing or has invalid 'output'. Provide the function result as a string. Example: {"tool_call_id": "call_abc123def456ghi789jkl012", "output": "The calculation result is 42"}.`)
                    };
                }
            }
        }
    ...
    async execute(args) {
        try {
            ...
            // Use generic request type
            const genericRequest = {
                toolOutputs: submitData.tool_outputs?.map((output) => ({
            ...
            throw this.createExecutionError(`Failed to submit tool outputs: ${error instanceof Error ? error.message : 'Unknown error'}`, error);
        }
    }
}
/**
 ...
 */
export function createRunHandlers(context) {
    return {
